 ...
 * @return void
*/ 
void GuardarPath (char *titulo, char *pathlibro, char *carpeta)
{
  int i, k, lentitulo;
  char tituloaux[TAM_DATOS];
  
  lentitulo = strlen(titulo);
  strcpy ( tituloaux, titulo);
  strcpy ( pathlibro, carpeta);
  
  *(tituloaux+lentitulo-1) = '\0' ;
  
  for( i = 0, k = 0 ; i < lentitulo ; i++)
  {
    if( *(tituloaux+i) != ' ')
    {
      *(tituloaux+k) = *(tituloaux+i);
      k++;
    }      
  }
  *(tituloaux+k+1) = '\0';
  strcat( pathlibro, tituloaux);  
}
/**
...
void FuncionUser (int sockfd, struct in_addr ip_cliente)
{
  ...
  do
  {
	...
	exit(EXIT_FAILURE);
      ...
      if( send(sockfd, "Si hubo coincidencias\n", TAM_DATOS, 0) == -1)
      {
	perror("Send");
	exit(EXIT_FAILURE);
      }
  ...
  GuardarPath (libroelegido, libroelegidopath, PATH_SINOPSIS);
  ...
  {
    printf("Enviando Sinopsis al cliente %s...\n", inet_ntoa(ip_cliente) );
    if( send(sockfd, "Se pudo abrir el Archivo", TAM_DATOS, 0) == -1)
    {
      perror("Send");
      exit(EXIT_FAILURE);
    }
  ...
  CargarBuffer (fd_libro, bufferlibro);
  ...
  if( j == 1 )
  {
    printf("El cliente %s quiso descargar el libro: %s", inet_ntoa(ip_cliente), libroelegido);
    
    GuardarPath (libroelegido, libroelegidopath, PATH_BIBLIOTECA);
    ...
    fclose(fd_libro);  
      ...
      printf("El cliente %s no quiso descargar el libro: %s\n", inet_ntoa(ip_cliente), libroelegido );
  BorrarLista( (&nodoprimerobusqueda) );
}
/**
